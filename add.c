...
void add(){
	...
	while (1){//让程序一直运行直到结束
		...
		switch (c){
			case 1://确认输入 
				printf("请输入新学生信息（学号 学生姓名 学生成绩 学生籍贯 学生出生日期（年 月 日））：\n");//打印提示语 
				scanf("%ld %s %f %s %d %d %d",&num,name,&price,address,&year,&month,&day);//输入新学生信息 
				fprintf(pf,"%ld %s %.0f %s %d %d %d\n",num,name,price,address,year,month,day);//将新学生信息输入至txt文件 
				printf("输入成功！是否继续输入......\n");//打印提示语 
				break;
			case 0://返回 
				fclose(pf);//关闭文件 
